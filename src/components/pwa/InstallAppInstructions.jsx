export function InstallAppInstructions({
  ...
  if (isInAppBrowser) {
    return (
      <p
        className={`${alignClass} ${compact ? 'text-xs leading-relaxed text-amber-950' : 'text-sm leading-relaxed text-ink'}`}
      >
  ...
  if (isIos) {
    return (
      <div
        className={`${alignClass} ${compact ? 'space-y-1.5 text-xs leading-relaxed text-ink' : 'space-y-2 text-sm leading-relaxed text-muted'}`}
      >
  ...
  return (
    <p
      className={`${alignClass} ${compact ? 'text-xs leading-relaxed text-ink' : 'text-sm leading-relaxed text-muted'}`}
    >
